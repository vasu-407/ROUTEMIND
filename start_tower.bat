...
start "Optimization API" cmd /k "cd /d %~dp0logistics_control_tower && uvicorn api.app:app --host 0.0.0.0 --port 8000"

echo [2/4] Starting ML Prediction Service (Port 8001)...
start "ML API" cmd /k "cd /d %~dp0logistics_control_tower && python ml/ml_api.py"

echo [3/4] Starting Node.js API Gateway (Port 3000)...
start "Node.js Gateway" cmd /k "cd /d %~dp0backend && node src/app.js"

echo [4/4] Starting React Frontend...
start "React UI" cmd /k "cd /d %~dp0logistics_control_tower\frontend && npm run dev"
