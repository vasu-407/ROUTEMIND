...
class RouteOptimizer:
    ...
    def optimize(self, route: Route, cache=None, demo_n: int = None) -> dict:
        ...
        # ── Handle Open vs Return-to-Depot routing ───────────────────────────
        route_mode = getattr(route, "route_mode", "return_to_depot")
        dummy_index = None

        if route_mode == "open":
            dummy_index = len(data["time_matrix"])
            for row in data["time_matrix"]:
                row.append(0)  # 0 time from any node to dummy
            data["time_matrix"].append([0] * (dummy_index + 1))
            ...
            manager = self.pywrapcp.RoutingIndexManager(
                len(data["time_matrix"]), data["num_vehicles"], [data["depot"]], [dummy_index]
            )
        else:
            manager = self.pywrapcp.RoutingIndexManager(
        ...
        with open("solve_debug.txt", "a") as f: f.write("Creating RoutingModel...\n"); f.flush()
        ...
        with open("solve_debug.txt", "a") as f: f.write("RegisterTransitCallback...\n"); f.flush()
        transit_callback_index = routing.RegisterTransitCallback(time_callback)
        routing.SetArcCostEvaluatorOfAllVehicles(transit_callback_index)

        with open("solve_debug.txt", "a") as f: f.write("AddDimension Time...\n"); f.flush()
        ...
        with open("solve_debug.txt", "a") as f: f.write("Setting time windows...\n"); f.flush()
        for node_index in range(len(data["time_matrix"])):
            if node_index == data["depot"]:
                continue
            index = manager.NodeToIndex(node_index)
            if index == -1: # this happens if the node isn't valid, but let's check
                continue
        ...
        with open("solve_debug.txt", "a") as f: f.write("Setting demand dimension...\n"); f.flush()
        ...
        with open("solve_debug.txt", "a") as f:
            f.write(f"Before SolveWithParameters (route_mode={route_mode})\n")
            f.flush()

        solution = routing.SolveWithParameters(search_parameters)

        with open("solve_debug.txt", "a") as f:
            f.write(f"After SolveWithParameters, solution={bool(solution)}\n")
            f.flush()
        ...
        if solution:
            with open("solve_debug.txt", "a") as f:
                f.write("Entering _extract_solution\n")
                f.flush()
            result = self._extract_solution(
                manager, routing, solution, stops_list, data,
                service_times, total_route_demand, execution_time_ms, dummy_index
            )
    ...
    def _extract_solution(
        ...
        while not routing.IsEnd(index):
            ...
            if node_index != dummy_index:
                optimized_order.append(stops_list[node_index])
            previous_index = index
            index = solution.Value(routing.NextVar(index))
            if not routing.IsEnd(index):
                next_node = manager.IndexToNode(index)
                if next_node != dummy_index and node_index != dummy_index:
